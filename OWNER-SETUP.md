...
- **Daily reading order:** seven layouts per weekly Observatory edition. A new lead, highlights and starting batch are chosen each calendar day in Europe/Amsterdam; original dates, source links and classifications stay intact. Every current story remains accessible. Readers can switch to newest reporting or measured readership.
- **Culture, 03:25 UTC daily:** select five works from `data/culture/library.sqlite`; **15:25 UTC** provides a second publishing opportunity using the same day's selection. The first calendar contains 400 entries for each of the five categories, used once per category over 400 days. A build can select today's works even if the collection job was delayed.
- **Stored sources:** 400 museum artworks and their cached JPGs; 400 poems; 400 short readings; 400 contextual quotations; 400 attributed music recordings. The artwork comes from the Cleveland Museum of Art's CC0 collection. Text comes from credited historical editions at Project Gutenberg and PoetryDB. Music records link to Wikimedia Commons and retain composer/performer and recording credits with source-declared reuse terms. Recording audio loads from its original host on Play. Nationality or cultural background and edition/translator details remain visible. Daily selection makes no source HTTP calls and needs no new database service.
- **More voices:** source-linked discoveries include Clarice Lispector, Kafka, Dalí, Orwell, Nobel laureates and Tolstoy's documented 1902 literature nomination. These references link to the original institution; protected work is not copied. Nominations are labelled separately from prizes.
- **Collection maintenance:** the five `data/culture/library-*.json` record lists are the editable source for the SQLite database. After a deliberate source/credit edit, run `python scripts/build_culture_library.py`, then run tests and commit the data and database together. No daily scraping is needed. After 400 days the calendar cycles; it can be extended with additional sourced entries before then.
